...
from model.architecture8 import LitAutoEncoder
from model.model_student import LitAutoEncoder_student
...
teacher_spath = output_path + '/Teacher'  # folder for saving teacher model and feature planes.
student_spath = output_path + '/Student'  # folder for saving student model and feature planes.

if not os.path.exists(teacher_spath):
    os.makedirs(teacher_spath, mode=0o777, exist_ok=True)
if not os.path.exists(student_spath):
...
trainer_distillation = L.Trainer(enable_model_summary=False, max_epochs=max_epochs, check_val_every_n_epoch=1,
                                 callbacks=[EarlyStopping(monitor="val_loss", mode="min", patience=5),
                                            lr_monitor])
...
features = np.reshape(unmasked_features, (h, w, comp_coeff))
for j in range(comp_coeff // 3):
    cv.imwrite(student_spath + '/plane' + '_' + str(j) + '.jpg',
               features[..., 3 * j:3 * (j + 1)].astype(np.uint8))
